...
export default function PhotoEditPage() {
  ...
  const makeDetailUrl = () => {
    ...
    return query
      ? `/photo-detail/${params.id}?${query}`
      : `/photo-detail/${params.id}`;
  };
  ...
  const makeUpdatedDetailUrl = (targetId) => {
    ...
    return query
      ? `/photo-detail/${targetId}?${query}`
      : `/photo-detail/${targetId}`;
  };
  ...
  const setupEditForm = (loadedPhotos, urlGroup, urlMember, urlType, urlYear) => {
    ...
    if (firstPhoto) {
      ...
      setCompleteType(
        firstPhoto.completeType || getDefaultCompleteType(fixedGroup)
      );
    } else {
  ...
  const buildUpdates = () => {
    ...
    const hiddenStandardPoses = allStandardPoseList.filter(
      (pose) => !visibleStandardPoses.includes(pose)
    );

    const hiddenHasCount = hiddenStandardPoses.some(
      (pose) => Number(normalPoseCounts[pose] || 0) > 0
    );
    ...
    const normalPoseUpdates = allStandardPoseList
      .map((pose) => {
        ...
        return {
          ...
          pose,
          count: isVisible
            ? normalPoseCounts[pose] === "" || normalPoseCounts[pose] == null
              ? 0
              : Number(normalPoseCounts[pose])
            : 0,
    ...
    const filledOtherPoses = otherPoses.filter(
      (item) => item.id || item.name.trim() || item.count || item.image
    );
  ...
  const applyUpdatesToPhotos = (sourcePhotos, updates) => {
    ...
    const updatedPhotos = sourcePhotos.map((photo) => {
      ...
      const updateById = updates.find(
        (item) => item.id && String(item.id) === photoId
      );

      const updateByPose = updates.find(
        (item) => !item.id && item.pose === photo.pose
      );
    ...
    return {
      firstSavedId,
      photos: normalizePhotos(updatedPhotos),
    };
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const filteredLocalPhotos = localResult.photos.filter(
        (photo) => Number(photo.count || 0) > 0
      );
      ...
      <div className="w-full max-w-md md:max-w-3xl lg:max-w-4xl mx-auto">
        ...
        <div className="bg-zinc-900 border border-zinc-700 rounded-3xl p-4 mb-5">
          ...
          <div className="grid gap-2 text-sm text-zinc-300">
            <p>
              <span className="text-zinc-500">グループ：</span>
              {group}
            </p>
            <p>
            ...
            <p>
              <span className="text-zinc-500">期生：</span>
              {generation}
            </p>
          </div>
        </div>
              ...
              {normalPoseList.map((pose) => (
                ...
                >
                  <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,104px)] gap-3 items-start">
                    ...
                    <CountSelector
                      value={normalPoseCounts[pose] || ""}
                      onChange={(value) =>
                        handleNormalPoseCountChange(pose, value)
                      }
                      allowZero={true}
                    />
                  </div>
                  ...
                  >
                    <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,104px)] gap-3 items-start">
                      ...
                      <CountSelector
                        ...
                        allowZero={Boolean(otherPose.id)}
                      />
                    </div>
